...
public class PrsConsoleApp {
		...
		while (menuOpt != 6) {
			...
			case 2:  	// get a user
				if (users.size() == 0) {
					...
					userIndex = getUserNameIndex(users);
					if (userIndex != -1) {
						printHeadings();
			...
			case 4:		// edit a user
				if (users.size() == 0) {
					...
					if (userIndex != -1) {
						String newEmail = Console.getString("Enter new email address: ");
						users.get(userIndex).setEmail(newEmail);
	...
	public static void insertInitialUsers(List<User> users) {
	...
	public static void addNewUser(List<User> users) {
		...
		boolean userNameExists = UserNameExists(userName, users);
	...
	public static boolean UserNameExists(String s, List<User> users) {
	...
	public static int getUserNameIndex(List<User> users) {
		boolean userFound = false;
		int index = 0;
